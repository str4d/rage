...
err-read-rsa-modulus-too-large =
    ...
    ---------------------
    {-openssh} поддерживает различные размеры модуля RSA, но {-rage} поддерживает только
    публичные ключи максимум {$max_size} бит, чтобы предотвратить условия отказа в обслуживании (DoS)
    при шифровании для ненадежных публичных ключей.
...
ssh-insecure-key-format =
    ...
    -----------------------------
    До версии {-openssh} 7.8, если при создании нового ключа 
    DSA, ECDSA или RSA был установлен пароль, {-ssh-keygen} шифровал ключ, используя зашифрованный
    ...
    Если вы используете версию {-openssh} между 6.5 и 7.7 (например, стандартную версию 
    {-openssh} в Ubuntu 18.04 LTS), вы можете использовать следующую команду 
    для создания ключей с использованием нового формата:
...
ssh-unsupported-cipher =
    ...
    ----------------------------------------
    {-openssh} внутренне поддерживает несколько различных шифров для зашифрованных ключей,
    но только некоторые из них генерируются напрямую. {-rage} поддерживает все
    шифры, которые может генерировать {-ssh-keygen},
    и обновляется по мере необходимости для поддержки нестандартных шифров.
    Ваш ключ использует в настоящее время неподдерживаемый шифр ({$cipher}).
...
ssh-unsupported-key-type =
    ...
    подмножество этих для обратной совместимости, в частности типы ключей '{-ssh-rsa}'
    и '{-ssh-ed25519}'. Этот SSH ключ использует неподдерживаемый тип ключа
    '{$key_type}'.

ssh-unsupported-security-key =
    Неподдерживаемый аппаратный аутентификатор SSH
    --------------------------------------
    Версия {-openssh} 8.2p1 добавила поддержку аппаратных аутентификаторов {-fido-u2f},
    включая аппаратные ключи безопасности, такие как {-yubikeys}. {-rage} не работает с
    этими типами ключей SSH, потому что их протокол не поддерживает шифрование. 
    Этот SSH ключ использует несовместимый тип '{$key_type}'.

    Если у вас есть совместимый аппаратный ключ безопасности, вы должны использовать этот плагин:

    {$age_plugin_yubikey_url}

    Аппаратный ключ безопасности, используемый одновременно с {-openssh} и этим плагином
    будет иметь отдельный публичный SSH-ключ и получателя шифрования {-age}, потому что
    плагин реализует протокол {-piv}.
